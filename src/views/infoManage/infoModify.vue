          ...
          <div class="area">
            ...
            >
              ...
              <el-form-item label="投资年限" prop="touziyear">
                <el-select v-model="infoForm.touziyear" placeholder="请选择投资年限" size="mini">
...
export default {
  data() {
    // 邮箱验证，附带callback(),是在明确通过验证的情况下去掉输入框上的loading
    ...
    // 新密码验证
    let validataSurepassword = (rule, value, callback) => {
      if(value === "") {
        callback(new Error("请输入新密码"));
    ...
    return {
      ...
      pwdRules: {
        ...
        surepassword: [
          {
            required: true,
            validator: validataSurepassword,
            trigger: "blur"
          }
        ]
      },
          ...
          if(formName === "pwdForm") {
            ...
            for(let key in this.phoneForm) {
              if(this.phoneForm[key] === "") {
                this.showMessage("warning", "请你选择手机服务");
                return;
              }
            }
            ...
            for(let key in this.infoForm) {
              if(this.infoForm[key] === "") {
                this.showMessage("warning", "请你修改相关信息")
                return
              }
            }
          }
